...
フィルタリング例（簡単な書き方）:
  $0 sandbox                   # 名前に"sandbox"を含むアカウント（推奨）
  $0 prod                      # 名前に"prod"を含むアカウント
  $0 123456789012              # 特定のアカウントID（部分一致）
  $0 sandbox --detailed        # sandboxアカウントの詳細表示
  $0 freee -s ALL              # 名前に"freee"を含む全ステータスのアカウント
  
フィルタリング例（従来の書き方も利用可能）:
...
get_sorted_accounts() {
    ...
    local accounts_data cache_stderr
    # aws_cache.shの標準エラー出力をキャプチャして、キャッシュ状態を確認
    if [[ "$show_message" == "true" ]]; then
        # 標準エラー出力をキャプチャしつつ、表示も行う
        accounts_data=$("$AWS_CACHE_SCRIPT" "${cache_args[@]}" -- "${aws_command[@]}" 2> >(tee /dev/stderr))
    else
        # メッセージ表示なしの場合は標準エラー出力を破棄
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -*)
                ...
                ;;
            *)
                # オプションではない引数はフィルターパターンとして扱う
                if [[ -z "$filter_pattern" ]]; then
                    filter_pattern="$1"
                else
                    echo "❌ エラー: フィルターパターンは1つだけ指定できます" >&2
                    echo "既に指定済み: '$filter_pattern', 新しい引数: '$1'" >&2
                    exit 1
                fi
                shift
                ;;
